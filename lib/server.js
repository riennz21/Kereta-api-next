...
function createExpressApp() {
  ...
  app.get("/purchases", async (req, res, next) => {
    try {
      ...
      if (req.query.page) {
        ...
        const summary = await getPurchaseSummary(period, startDate, endDate, status);
        return res.json({
  ...
  app.get("/purchases/summary", async (req, res, next) => {
    try {
      ...
      const summary = await getPurchaseSummary(actualPeriod, startDate, endDate, status);
      const shouldShowBreakdown = actualPeriod !== "all" && actualPeriod !== "custom"
        ? true
        : isCustomRange;
      const dailyBreakdown = shouldShowBreakdown
        ? await getPurchaseSummaryByDate(actualPeriod, startDate, endDate, status)
        : [];
