...
const POSTGRES_PURCHASE_COLUMNS = `
  id,
  kereta_id,
  nama_kereta,
  asal,
  ...
  status_pembayaran,
  kode_booking,
  to_char(tanggal_pembelian, 'YYYY-MM-DD HH24:MI:SS') AS tanggal_pembelian,
    ...
    if (provider === "postgres") {
      const result = await getPgPool().query(
        `
          ...
          RETURNING ${POSTGRES_PURCHASE_COLUMNS}
        `,
        values,
      );
...
export async function getPurchaseByBookingCode(kodeBooking) {
  ...
  if (provider === "postgres") {
    const result = await getPgPool().query(
      `SELECT ${POSTGRES_PURCHASE_COLUMNS} FROM pembelian WHERE kode_booking = $1`,
      [kodeBooking],
    );
...
export async function getPurchaseById(id) {
  ...
  if (provider === "postgres") {
    const result = await getPgPool().query(
      `SELECT ${POSTGRES_PURCHASE_COLUMNS} FROM pembelian WHERE id = $1`,
      [id],
    );
    return result.rows[0] || null;
  }
...
function buildPurchaseSearchFilter(dbProvider, search, status, startIndex = 1) {
  const clauses = [];
  const params = [];
  ...
  if (search && search.trim()) {
    if (dbProvider === "postgres") {
      clauses.push(`(nama_kereta ILIKE $${index} OR kode_booking ILIKE $${index + 1})`);
      index += 2;
    } else {
  ...
  if (status && status !== "all") {
    if (dbProvider === "postgres") {
      clauses.push(`status_pembayaran = $${index}`);
      index += 1;
    } else {
...
function buildFullTextFilter(dbProvider, search, startIndex = 1) {
  if (!search || !search.trim()) {
  ...
  const orClauses = dbProvider === "postgres"
    ? fields.map((f, idx) => `${f} ILIKE $${startIndex + idx}`)
    : fields.map((f) => `${f} LIKE ?`);
...
export async function searchPurchases(search, options = {}) {
  await ensureInitialized();
  const { clause: searchCondition, params: searchParams } = buildFullTextFilter(provider, search);
  ...
  if (provider === "postgres") {
    ...
    const dataResult = await getPgPool().query(
      `SELECT ${POSTGRES_PURCHASE_COLUMNS} FROM pembelian WHERE 1=1${searchCondition} ORDER BY tanggal_pembelian DESC LIMIT $${searchParams.length + 1} OFFSET $${searchParams.length + 2}`,
...
export async function getAllPurchases(filters = {}) {
  ...
  const { clause: dateCondition, params: dateParams, nextIndex } = buildPurchaseDateFilter(provider, period, startDate, endDate);
  const { clause: searchCondition, params: searchParams } = buildPurchaseSearchFilter(provider, search, status, nextIndex);

  const pageLimit = Math.min(Math.max(1, Number(limit) || 50), 200);
  const pageOffset = Math.max(0, Number(offset) || 0);

  if (provider === "postgres") {
    let query = `SELECT ${POSTGRES_PURCHASE_COLUMNS} FROM pembelian WHERE 1=1 ${dateCondition}${searchCondition} ORDER BY tanggal_pembelian DESC`;
    if (pageLimit) {
...
export async function getPaginatedPurchases(filters = {}, options = {}) {
  ...
  if (provider === "postgres") {
    ...
    const offset = (page - 1) * perPage;
    const dataResult = await getPgPool().query(
      `SELECT ${POSTGRES_PURCHASE_COLUMNS} FROM pembelian WHERE 1=1 ${dateCondition}${searchCondition} ORDER BY tanggal_pembelian DESC LIMIT $${allParams.length + 1} OFFSET $${allParams.length + 2}`,
...
function buildPurchaseDateFilter(dbProvider, period, startDate, endDate, startIndex = 1) {
  if (period === "today") {
    return {
      clause: dbProvider === "postgres"
        ? "AND date(tanggal_pembelian) = CURRENT_DATE"
        : "AND date(tanggal_pembelian) = date('now', 'localtime')",
      params: [],
  ...
  if (period === "week") {
    return {
      clause: dbProvider === "postgres"
        ? "AND tanggal_pembelian >= CURRENT_DATE - INTERVAL '7 days'"
        : "AND tanggal_pembelian >= datetime('now', '-7 days', 'localtime')",
      params: [],
      nextIndex: startIndex,
    };
  }
  if (period === "month") {
    return {
      clause: dbProvider === "postgres"
        ? "AND tanggal_pembelian >= CURRENT_DATE - INTERVAL '30 days'"
        : "AND tanggal_pembelian >= datetime('now', '-30 days', 'localtime')",
      params: [],
      nextIndex: startIndex,
    };
  }
  if (startDate && endDate) {
    return {
      clause: dbProvider === "postgres"
        ? `AND date(tanggal_pembelian) >= $${startIndex} AND date(tanggal_pembelian) <= $${startIndex + 1}`
        : "AND date(tanggal_pembelian) >= ? AND date(tanggal_pembelian) <= ?",
...
function buildPurchaseWhereClause(period, startDate, endDate, status) {
  const dateFilter = buildPurchaseDateFilter(provider, period, startDate, endDate);
  const statusFilter = buildPurchaseSearchFilter(provider, "", status, dateFilter.nextIndex);
  return {
    whereClause: `WHERE 1=1 ${dateFilter.clause}${statusFilter.clause}`,
    params: [...dateFilter.params, ...statusFilter.params],
  };
}

export async function getPurchaseSummary(period, startDate, endDate, status) {
  await ensureInitialized();
  const { whereClause, params } = buildPurchaseWhereClause(period, startDate, endDate, status);
...
export async function getPurchaseSummaryByDate(period, startDate, endDate, status) {
  ...
  if (provider === "postgres") {
    const result = await getPgPool().query(
      `
        SELECT
          to_char(tanggal_pembelian::date, 'YYYY-MM-DD') AS tanggal,
...
export async function getPurchaseRevenueByStatus(period, startDate, endDate) {
  await ensureInitialized();
  const { whereClause, params } = buildPurchaseWhereClause(period, startDate, endDate);
