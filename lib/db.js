      ...
      `
        INSERT INTO pembelian
          (kereta_id, nama_kereta, asal, tujuan, kelas, harga_satuan, jumlah_tiket,
           ...
           status_pembayaran, tanggal_keberangkatan)
...
export async function getAllPurchases(filters = {}) {
  await ensureInitialized();
  const { period, startDate, endDate, limit, offset } = filters;
  ...
  if (provider === "postgres") {
    let query = `SELECT * FROM pembelian WHERE 1=1 ${dateCondition} ORDER BY tanggal_pembelian DESC`;
    if (pageLimit) {
      query += ` LIMIT $${dateParams.length + 1} OFFSET $${dateParams.length + 2}`;
    }
    const result = await getPgPool().query(query, [...dateParams, pageLimit, pageOffset]);
    return result.rows;
  }
  ...
  return db.prepare(query).all(...dateParams, pageLimit, pageOffset);
}
...
export async function getPaginatedPurchases(filters = {}, options = {}) {
  ...
  if (provider === "postgres") {
    const countResult = await getPgPool().query(
      `SELECT COUNT(*)::int AS total FROM pembelian WHERE 1=1 ${dateCondition}`,
      dateParams,
    );
    const total = countResult.rows[0]?.total || 0;
    const totalPages = Math.max(1, Math.ceil(total / perPage));
    ...
    const dataResult = await getPgPool().query(
      `SELECT * FROM pembelian WHERE 1=1 ${dateCondition} ORDER BY tanggal_pembelian DESC LIMIT $${dateParams.length + 1} OFFSET $${dateParams.length + 2}`,
      [...dateParams, perPage, offset],
    );
    return { rows: dataResult.rows, total, page, perPage, totalPages };
  }

  const db = getSqliteDb();
  const countRow = db.prepare(`SELECT COUNT(*) AS total FROM pembelian WHERE 1=1 ${dateCondition}`).get(...dateParams);
  const total = countRow?.total || 0;
  const totalPages = Math.max(1, Math.ceil(total / perPage));
  const page = Math.min(requestedPage, totalPages);
  const offset = (page - 1) * perPage;
  const rows = db
    .prepare(`SELECT * FROM pembelian WHERE 1=1 ${dateCondition} ORDER BY tanggal_pembelian DESC LIMIT ? OFFSET ?`)
    .all(...dateParams, perPage, offset);
