...
// Helper: execute a query across providers.
// pgQuery: PostgreSQL SQL with $1, $2 placeholders + params array.
// sqliteQuery: SQLite SQL with ? placeholders + params array.
// For SELECT, use dbQueryAll or dbQueryGet instead.
async function dbExec(pgQuery, sqliteQuery, params = []) {
  if (isPostgres()) {
    await getPgPool().query(pgQuery, params);
  } else {
    (await getSqliteDb()).prepare(sqliteQuery).run(...params);
  }
}

// Helper: run a query that returns multiple rows.
async function dbQueryAll(pgQuery, sqliteQuery, params = []) {
  if (isPostgres()) {
    const result = await getPgPool().query(pgQuery, params);
    return result.rows;
  }
  return (await getSqliteDb()).prepare(sqliteQuery).all(...params);
}

// Helper: run a query that returns a single row.
async function dbQueryGet(pgQuery, sqliteQuery, params = []) {
  if (isPostgres()) {
    const result = await getPgPool().query(pgQuery, params);
    return result.rows[0] || null;
  }
  return (await getSqliteDb()).prepare(sqliteQuery).get(...params) || null;
}
...
export async function getTrainById(id) {
  ...
  return mapTrainRecord((await getSqliteDb()).prepare("SELECT * FROM kereta WHERE id = ?").get(id));
}
...
export async function deleteTrain(id) {
  ...
  (await getSqliteDb()).prepare("DELETE FROM kereta WHERE id = ?").run(id);
  return existing;
}
...
export async function getScheduleRows() {
  ...
  return (await getSqliteDb()).prepare("SELECT nama, tanggal, jam FROM kereta ORDER BY tanggal, jam").all();
}
...
export async function getStatusRows() {
  ...
  return (await getSqliteDb()).prepare("SELECT nama, status FROM kereta ORDER BY nama").all();
}
...
export async function getPurchaseByBookingCode(kodeBooking) {
  ...
  return (await getSqliteDb()).prepare("SELECT * FROM pembelian WHERE kode_booking = ?").get(kodeBooking) || null;
}
...
export async function getPurchaseById(id) {
  ...
  return (await getSqliteDb()).prepare("SELECT * FROM pembelian WHERE id = ?").get(id) || null;
}
